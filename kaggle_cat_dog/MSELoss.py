...
def MSEInput(src):
    return torch.nn.Softmax(dim=1)(src)


def MSETarget(src):
    src = src.reshape(-1, 1)
    return torch.zeros(src.shape[0], 2).to("cuda:0").scatter_(1, src, 1)
...
if __name__ == "__main__":
    ...
    writer_train = SummaryWriter(log_dir="./board/MSELoss/train")
    writer_valid = SummaryWriter(log_dir="./board/MSELoss/valid")
    ...
    for epoch in range(7):
        ...
        for i, (imgs, labels) in enumerate(train_loader):
            ...
            prediction = cnn(imgs)
            lossCrossEntropy = nn.CrossEntropyLoss()(prediction, labels)
            loss_all.append(lossCrossEntropy)
            prediction = MSEInput(prediction)
            labels = MSETarget(labels)
            loss = nn.MSELoss()(prediction, labels)
            if i % 20 == 0:
                print('train i = %d, epoch = %d, loss = %f' % (i, epoch, torch.mean(torch.tensor(loss_all))))
